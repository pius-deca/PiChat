...
public class PasswordServiceImpl implements PasswordService {
  ...
  @Autowired
  public PasswordServiceImpl(UserRepository userRepository, EmailSenderService emailSenderService,
      PasswordTokenRepository passwordTokenRepository, TokenService tokenService, PasswordEncoder passwordEncoder) {
    ...
    try {
      ...
      if (!userByEmail.isPresent()) {
        if (userByUsername.isPresent()) {
          ...
          this.emailSenderService.sendMail(userByUsername.get().getEmail(), "Reset pichat password",
              "Hi " + userByUsername.get().getUsername() + "\n"
      ...
      this.emailSenderService.sendMail(userByEmail.get().getEmail(), "Reset pichat password",
          "Hi " + userByEmail.get().getUsername() + "\n" + "A request was made to reset your pichat password" + "\n"
              + "http://localhost:3000/account/password/reset?token=" + token.getToken());
      return "Thanks! Please check your email for a link to reset your password.";
    } catch (Exception e) {
